...
function wp_trigger_netlify_build_settings_init() {
  ...
  add_settings_field(
    'wp_trigger_netlify_use_multiple_hooks',
    __('Use multiple build hooks', 'wordpress'),
    'wp_trigger_netlify_use_multiple_hooks_render',
    'wp_trigger_netlify_build',
    'wp_trigger_netlify_build_section'
  );
}
...
function wp_trigger_netlify_use_multiple_hooks_render() {
  $options = get_option( 'wp_trigger_netlify_build_settings' );
  ?>
  <input type='checkbox' name='wp_trigger_netlify_build_settings[wp_trigger_netlify_use_multiple_hooks]' value="1" <?php if($options['wp_trigger_netlify_use_multiple_hooks']){?> checked <?php } ?> >
  <span class="description"><?php esc_attr_e( 'Use multiple build hooks, comma separated ', 'WpAdminStyle' ); ?></span><br>
  <?php
}
...
function wp_trigger_netlify_build_dashboard_status() {
    ...
    $build_hooks = explode(",", $options['wp_trigger_netlify_build_webhook_url']);
    if(!$options['wp_trigger_netlify_use_multiple_hooks'] || !is_array($build_hooks)) {
      $markup .= 'jQuery.ajax({ 
        type: "POST",
